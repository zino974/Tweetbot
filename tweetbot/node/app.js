...
const 
 ...
 var TwitterClient = new Twitter ({
    consumer_ker: config.get('twitterkey'),
...
function receivedMessage(event) {
 ...
 var greeting = new RegExp(/^(yo|ola|hola|howdy|hi|hey|hello |salut|bonjour|)/i);
 var hashtag = new RegExp(/#\w+/i);

  if (messageText.match(greeting)) {
    sayHello(senderID);
  } else if (messageText.match(hashtag)) { 
    var trend = messageText.match(hashtag)[0].replace ("#", "%23");
    getTweets(senderID, trend);
  } else if (messageText.match('image')) {
    sendImagetMessage(senderID);
  ...
  } else if (messageText.match('quick replay')) {
    sendQuickReplay(senderID);
  } else {
    sendTextMessage(senderID, messageText);
  }  
 } else if (messageAttachments) {
       sendTextMessage(senderID, "Message with attachments receive");
 }  
}
  ...
  TwitterClient.get('search/tweets', {q: trend}, function(error, body, response) {
    if (error) {
      console.log(error);
          ...
          var illustratedTweets = body.status.filter(t => { return t.entities.media}).slice(0.3);
          if (illustratedTweets.length > 0) {
            var cards = illustratedTweets.map(t => {
              return {
                title: "@" + t.user.screen_name,
                subtitle: t.text,
                item_url: "http://Twitter.com/" + t.user.id_str + "/status/" + t.id_str,
